...
public abstract class BigCakePresenter<V extends ViewCallbacks, R extends Router> extends Presenter<V> {
  @NonNull private final R routerProxy = BigCake.createRouter(null, getClass());
   ...
   * @since 0.11.0
   */
  protected BigCakePresenter(@NonNull V view, @NonNull R router) {
    super(view);
   ...
   * @since 0.11.0
   */
  protected BigCakePresenter(@NonNull V view) {
    super(view);
  }
   ...
   * @since 0.11.0
   */
  protected BigCakePresenter(@NonNull R router) {
    requireNotNull(router);
    getProxy(routerProxy).set(router);
  }
